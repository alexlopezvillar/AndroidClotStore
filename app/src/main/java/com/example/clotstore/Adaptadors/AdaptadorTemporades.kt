...
import com.example.clotstore.API.Temporada
import com.example.clotstore.ProductesMenu.ActivityTemporades
import com.example.clotstore.R

class AdaptadorTemporades(var llista: ArrayList<Temporada>) :
    RecyclerView.Adapter<AdaptadorTemporades.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.vista.setOnClickListener {
            val intent = Intent(holder.vista.context, ActivityTemporades::class.java)
            intent.putExtra("idTemporada", llista[position].idTemporada)
            intent.putExtra("nom", llista[position].nom)
            ContextCompat.startActivity(holder.vista.context, intent, null)
        }
    }

    fun updateLlista(llistanova: ArrayList<Temporada>) {
        llista = llistanova
