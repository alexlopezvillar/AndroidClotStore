...
import com.example.clotstore.API.Prenda
import com.example.clotstore.ProductesMenu.ActivityPrenda
import com.example.clotstore.R

class AdaptadorPrendas(var llista: ArrayList<Prenda>) :
    RecyclerView.Adapter<AdaptadorPrendas.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.vista.setOnClickListener {
            val intent = Intent(holder.vista.context, ActivityPrenda::class.java)

            intent.putExtra("idPrenda", llista[position].idPrenda)
            intent.putExtra("nom", llista[position].nom)
            ContextCompat.startActivity(holder.vista.context, intent, null)
        }
    }

    fun updateLlista(llistanova: ArrayList<Prenda>) {
        llista = llistanova
